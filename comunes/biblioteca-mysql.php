...
function borraTodo()
{
    ...
    if (!$pdo->query($consulta)) {
        ...
        $consulta = "CREATE TABLE $cfg[dbempleadosTabla]  (
                     id INTEGER UNSIGNED AUTO_INCREMENT,
                     nombre VARCHAR($cfg[dbempleadosTamNombre]),
                     apellidos VARCHAR($cfg[dbempleadosTamApellidos]),
                     telefono VARCHAR($cfg[dbempleadosTamTelefono]),
                     correo VARCHAR($cfg[dbempleadosTamCorreo]),
                     RFID VARCHAR($cfg[dbempleadosTamRFID]),
                     PRIMARY KEY(id)
                     )";

        if (!$pdo->query($consulta)) {
            print "    <p class=\"aviso\">Error al crear la tabla empleados. SQLSTATE[{$pdo->errorCode()}]: {$pdo->errorInfo()[2]}</p>\n";
        } else {
            print "    <p>Tabla empleados creada correctamente.</p>\n";
        }

        print "\n";

        $consulta = "CREATE TABLE $cfg[dblogsTabla]  (
                     id INTEGER UNSIGNED AUTO_INCREMENT,
                     nombre VARCHAR($cfg[dbempleadosTamNombre]),
                     apellidos VARCHAR($cfg[dbempleadosTamApellidos]),
                     RFID VARCHAR($cfg[dbempleadosTamRFID]),
